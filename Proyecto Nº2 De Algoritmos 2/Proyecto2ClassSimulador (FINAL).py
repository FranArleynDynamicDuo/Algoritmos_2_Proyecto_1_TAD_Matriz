...
while (p != [] or
    ...
    for PT in sim1.ListaProcesadores:
        ...
        for PC in PT.procesosEmpilados.elementos:
            PC.duracion += -1
            if (PC.duracion == 0 and PC.subprocesos == 0):
            ...
            elif (PC.duracion == 0 and PC.subprocesos != 0):

                PT.procesosEmpilados.elementos = []             
                PT.procesosEmpilados.elementos.append(PC.ListaSubprocesos[0])
                PC.ListaSubprocesos.pop(0)
                CON = PC.subprocesos
                PC.subprocesos = PC.subprocesos - 1
                ...
                print('-',Tiempo,'-','Termino el procesos',PC.nombre)
                print('-',Tiempo + 1,'-','se esta empilando el subproceso',
                      PT.procesosEmpilados.elementos[0].nombre)

                salida.write(str(Tiempo + 1))
                salida.write(' Iniciando proceso ')
                salida.write(PC.nombre)
                salida.write(' - ')
                salida.write(str(PC.subprocesos))
                salida.write(' en el procesador ')
                salida.write(str(PT.identificador))
                salida.write('\n')
                ...
                for imprimir in range(4):
                    ...
                    for imprimir2 in sim1.ListaProcesadores[imprimir].procesosEmpilados.elementos:
                        print('Los procesos del procesador ',imprimir + 1,
                              'son : ',imprimir2.nombre,
                              '- prioridad: ',imprimir2.prioridad)        
